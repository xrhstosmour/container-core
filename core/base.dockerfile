...
RUN apk update && apk upgrade && apk add --no-cache curl wget nano nss-tools nss \
    ...
    && mkdir -p /base/data && mkdir -p /base/data/shared_data && mkdir -p /base/data/archive_data \
    && mkdir -p /base/data/.certificates && mkdir -p /base/data/.keys && mkdir -p /base/data/.configurations \
